...
// ── Pastel surface palette (used consistently across all cards/sections) ──────
// These are the ONLY background fills used on cards and section containers.
// Do not introduce ad-hoc bg-* classes anywhere in this file.
const P_INDIGO = "#EEF0FF"; // pastel indigo
const P_YELLOW = "#FEF6E4"; // pastel amber
const P_RED    = "#FFF0EE"; // pastel red
const P_TEAL   = "#E6F8F5"; // pastel teal
const P_BLUE   = "#EBF4FF"; // pastel blue
...
// Programme cards — each with its own banner photo + tint colour
const FLAGSHIP_PROGRAMMES = [
  {
    ...
    // Tint: semi-transparent overlay colour on the image
    tint: "rgba(51,51,153,0.72)",
    // Accent for tab pill + stats
    accent: B_YELLOW,
    // Pastel surface for tab button when active
    pastelBg: P_INDIGO,
    accentText: B_INDIGO,
  },
  {
...
// EOEO showcase — replaces the old company list
const EOEO = {
  tag: "Each One Empowers One",
  tagColour: B_TEAL,
  tagPastel: P_TEAL,
  headline: "Become a TCS Literacy Champion",
  body: "TCS's intuitive digital literacy platform enables any Tata employee to empower another — with financial, digital, and functional literacy. Available in 9 Indian languages. Your cook, driver, security guard, or neighbour could be your beneficiary.",
  steps: [
    { num: "01", label: "Identify your beneficiary" },
    { num: "02", label: "Enroll them in the system" },
    { num: "03", label: "Teach at your own pace" },
    { num: "04", label: "Get rewarded + certificates" },
  ],
  cta: "Sign Up Now",
...
const HomeView = () => {
  ...
  return (
    // pt-28 = 64px nav bar + ~44px ticker bar below it = 108px total fixed header
    <div className="relative font-sans pt-28">
      ...
      {/* ════════════════════════════════════════════════════════════════════
          1. HERO
      ════════════════════════════════════════════════════════════════════ */}
      <section id="hero" className="relative min-h-[560px] flex items-center overflow-hidden bg-white">

        {/* Thin accent bar at very top of section */}
        <div className="absolute top-0 left-0 right-0 h-0.5 z-20 transition-colors duration-700"
          style={{ backgroundColor: slide.accent }} />

        {/* Photo layer — right-anchored with vignette */}
      ...
      {/* ════════════════════════════════════════════════════════════════════
          ...
          LEFT 70%: photo-backed programme card (image-led, text overlaid)
          RIGHT 30%: EOEO showcase panel
      ════════════════════════════════════════════════════════════════════ */}
      <section id="programmes" className={`${secBg(1)} py-10 px-6 md:px-12`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
            ...
            {/* ── LEFT 70% — image-led card like the reference screenshot ── */}
            <div className="lg:col-span-7">
              ...
              {/* Card — image on left (~45%), text on right (~55%), separated by gap. Like reference. */}
              <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm min-h-[320px] flex flex-col md:flex-row">

                {/* Image panel — with colour tint overlay */}
                <div className="relative md:w-[45%] min-h-[220px] md:min-h-[320px] shrink-0 overflow-hidden">
                  <img
                    src={prog.photo}
                    alt={prog.title}
                    ...
                    referrerPolicy="no-referrer"
                  />
                  {/* Slight colour tint — programme-specific */}
                  <div className="absolute inset-0 transition-all duration-700" style={{ backgroundColor: prog.tint }} />
                  {/* Programme label pill over image */}
                  <div className="absolute bottom-4 left-4 z-10">
                    <span className="text-xs font-bold px-3 py-1 rounded-full text-white/90"
                  ...
                  <div>
                    ...
                    <div className="flex gap-3 mb-6">
                      {[prog.stat1, prog.stat2].map((s) => (
                        <div key={s} className="rounded-xl px-4 py-2 text-center"
            ...
            <div className="lg:col-span-3 flex flex-col gap-0 bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
              {/* Header strip */}
              ...
              <div className="px-5 pb-5">
                <a href={EOEO.ctaUrl} target="_blank" rel="noopener noreferrer"
                  className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold text-white transition-all cursor-pointer hover:brightness-110"
                  style={{ backgroundColor: EOEO.tagColour }}>
      ...
      {/* ════════════════════════════════════════════════════════════════════
          ...
          Stats use pastel fills consistently.
      ════════════════════════════════════════════════════════════════════ */}
      <section id="numbers" className={`${secBg(3)} py-10 px-6 md:px-12`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            {/* Did You Know — pastel indigo surface instead of solid B_INDIGO for consistency */}
            ...
            {/* 3 Stats — pastel surface + strong left border */}
            <div className="flex flex-col gap-3">
              {HERO_STATS.map((s) => (
                <div key={s.label} className="flex items-center gap-5 rounded-2xl p-5 border border-slate-100 shadow-sm"
